...
const processPhotoMessage = async (ctx, message) => {
  ...
  if (message.photo && message.photo.length > 0) {
    let uploadContent = {};
    uploadContent[message.photo[0].file_id] = [];
    for(let photoItem of message.photo) {
      let uploadResponse = await uploadPhotoToTelegraph(ctx, photoItem);
      if(uploadContent[photoItem.file_id]) {
        uploadContent[photoItem.file_id].push({
          ...photoItem,
          ...uploadResponse
        }) 
      }
      else {
        uploadContent[photoItem.file_id] = [{
          ...photoItem,
          ...uploadResponse
        }]
      }
    ...
    return uploadContent[message.photo[0].file_id][uploadContent.length - 1];
  }
